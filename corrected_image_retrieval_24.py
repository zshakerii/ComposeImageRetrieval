...
def parse_cirr_sample(sample: dict) -> dict:
    """
    Parse CIRR annotations, including CIRR test1 JSON.

    Supported test1 structure:
        {
          "pairid": 12063,
    ...
    Important:
      * img_set.members is the candidate subset for CIRR test1 retrieval.
      * reference_rank is metadata only; it must not affect model ranking.
      * target_hard/target_soft are optional and are used only when local GT exists.
    """
    if not isinstance(sample, dict):
        raise ValueError("CIRR sample must be a JSON object.")
    ...
    if not isinstance(img_set, dict):
        raise ValueError(
            f"CIRR sample pairid={sample.get('pairid')!r} "
            "must contain an object in 'img_set'."
        )
    ...
    if not isinstance(members_raw, list):
        raise ValueError(
            f"CIRR sample pairid={sample.get('pairid')!r}: "
            "'img_set.members' must be a list."
        )
    ...
    if not reference_id:
        raise ValueError(
            f"CIRR sample pairid={sample.get('pairid')!r} has an empty reference."
        )

    if reference_id not in members:
        raise ValueError(
            f"CIRR sample pairid={sample.get('pairid')!r}: "
            f"reference '{reference_id}' is not present in img_set.members."
        )
...
def validate_cirr_annotations(
    ...
    if missing_caption:
        print("\n[WARN] Queries with empty captions will be skipped during inference:")
...
def evaluate_cross_modal(
    ...
    Returns rankings as well so CIRR test1 predictions can be saved even when
    no local ground-truth metrics are available.
    """
    id2idx = build_id_index(gallery_ids)
    ...
    for item in tqdm(data, desc=f"{model_name}: cache queries"):
        ...
        if ref_id not in ref_cache:
            ...
            ref_cache[ref_id] = (
                None if image is None else get_image_feature(image)
            )

        if caption not in text_cache:
            text_cache[caption] = (
                get_text_feature(caption) if caption else None
            )
    ...
    for item in tqdm(data, desc=f"{model_name}: ranking"):
        ...
        try:
            ...
            rankings[str(pairid)] = ranked_ids[:50]
            ...
            skip_reasons[reason] = skip_reasons.get(reason, 0) + 1
            print(
                f"[WARN] {model_name} failed for pairid={pairid!r}, "
...
def evaluate_clip_beta(
    ...
    Rankings are returned for CIRR test1 prediction generation.
    """
    id2idx = build_id_index(gallery_ids)
        ...
        for alpha in alphas:
            ...
            for item in tqdm(
                    ...
                    if ref_id not in ref_feature_cache:
                        ...
                        if image is None:
                            ...
                            try:
                                ...
                                print(
                                    f"[WARN] Reference embedding failed for "
                                    f"{ref_id}: {type(exc).__name__}: {exc}"
                                )
                                ref_feature_cache[ref_id] = None
                ...
                try:
                    ...
                    skip_reasons[reason] = skip_reasons.get(reason, 0) + 1
                    print(
                        f"[WARN] CLIP-beta failed for pairid={pairid!r}: {reason}"
                    )
                    continue
...
def save_cirr_test_predictions(
    ...
    """Save model rankings in a transparent query-level JSON format."""
    predictions = []

    for item in data:
        pairid = item.get("pairid")
        ranking = rankings.get(str(pairid), [])
...
def validate_cirr_gallery_coverage(
    ...
    """Verify that references/targets/members exist in the retrieval gallery."""
    ...
    missing_targets = sorted({
        get_relevant_id
        for item in data
        for get_relevant_id in [normalize_id(item.get("target_hard"))]
        if get_relevant_id and get_relevant_id not in gallery_set
    })
    ...
    for item in data:
        for member in item.get("members") or []:
            ...
            if member and member not in gallery_set:
    ...
    print(f"Missing target_hard IDs     : {len(missing_targets)}")
    print(f"Missing subset member IDs   : {len(missing_members)}")
    ...
    if missing_targets:
        print("First missing target_hard IDs:")
        for value in missing_targets[:20]:
            print(f"  {value}")

    if missing_members:
        print("First missing subset members:")
        for value in missing_members[:20]:
            print(f"  {value}")

    if require_ground_truth and (missing_targets or missing_members):
        raise RuntimeError(
            "CIRR validation gallery is incomplete. "
            "At least one target_hard/member is not present in the gallery. "
            "Use the correct CIRR validation image folder."
        )
...
def main():
    ...
    if args.dataset == "cirr":
        if args.require_cirr_gt and split_name == "test":
            raise RuntimeError(
                "--require_cirr_gt cannot be used with CIRR test split because "
                "public test annotations do not contain ground truth."
            )
    ...
    subset_protocol = (
        args.dataset == "cirr"
        and (args.cirr_subset or split_name == "test")
    )
