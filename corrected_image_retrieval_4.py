...
def load_searle_model(clip_model_name: str = "ViT-B/32"):
    ...
    On Windows, torch.hub's local loader can execute hubconf.py without
    making the cached repository root importable. SEARLE's hubconf.py
    imports `src.encode_with_pseudo_tokens`, so we add the repo root
    explicitly to sys.path first.
    """
    import sys
    ...
    try:
        ...
        if "src" in str(exc):
            raise RuntimeError(
                "SEARLE is cached but its `src` package cannot be imported. "
                f"Expected repository directory: {repo_dir}. "
                "Delete the cached SEARLE directory and rerun."
            ) from exc
        raise
