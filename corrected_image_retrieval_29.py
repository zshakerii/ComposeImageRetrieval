...
def rank_from_sims(
    ...
    """
    Rank against the FULL gallery.

    Important for this project:
        - Similarity is always computed against every gallery image.
        - restrict_ids is intentionally ignored.
        - Only reference images are excluded from the final ranking.
    """
    del restrict_ids  # Full-gallery retrieval is mandatory.
...
def _siglip_forward_image_features(inputs, model) -> torch.Tensor:
    """
    Return the projected SigLIP/SigLIP2 image embedding.

    IMPORTANT:
    - SigLIP2 ``get_image_features()`` may return a ModelOutput whose
      ``pooler_output`` is already pooled but not necessarily projected.
    - For the classic ``SiglipModel`` class, the correct projection is the
      *top-level* ``model.visual_projection`` applied to
      ``vision_model(...).pooler_output``.
    - Never feed a 2-D pooled tensor back through ``vision_model.head``; that
      pooling head expects the token sequence and causes the 3-D/2-D
      MultiheadAttention shape error seen with SigLIP.
    """

    # ------------------------------------------------------------
    # 1) Prefer the complete model forward when it exposes image_embeds.
    #    This is the cleanest path for SigLIP2/Siglip2Model.
    # ------------------------------------------------------------
    try:
        ...
        # Fall through to the explicit vision path.
        pass

    # ------------------------------------------------------------
    # 2) Get image/vision features.
    # ------------------------------------------------------------
    ...
    # Some versions expose image_embeds directly.
    image_embeds = getattr(output, "image_embeds", None)
    if torch.is_tensor(image_embeds):
        return image_embeds

    # ------------------------------------------------------------
    # 3) ModelOutput with pooler_output.
    #    For classic SiglipModel this is the pooled vision feature;
    #    apply model.visual_projection exactly once.
    # ------------------------------------------------------------
    ...
    if torch.is_tensor(pooler):
        ...
        if visual_projection is not None:
            return visual_projection(pooler)

        # SigLIP2 implementations may keep the projection on the
        # vision tower. Prefer image_embeds from the vision output.
    ...
    raise TypeError(
        ...
        f"{type(output).__name__}. Expected Tensor, image_embeds, "
        "or pooler_output with a valid projection."
    )


@torch.inference_mode()
def _siglip_forward_text_features(inputs, model) -> torch.Tensor:
    """Return the projected SigLIP/SigLIP2 text embedding."""

    # Clean path for full SigLIP/SigLIP2 model output.
    try:
        outputs = model(**inputs)
    ...
    if torch.is_tensor(pooler):
        # Classic SiglipModel uses top-level text_projection.
    ...
    raise TypeError(
        ...
        f"{type(output).__name__}. Expected Tensor, text_embeds, "
        "or pooler_output with a valid projection."
    )
...
def main():
    ...
    # IMPORTANT: Retrieval is ALWAYS performed against the FULL gallery.
    # CIRR group/members are annotation metadata only; they never restrict ranking.
    subset_protocol = False
    print("Retrieval protocol: FULL GALLERY (all gallery images)")
