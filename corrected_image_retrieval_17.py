...
def validate_cirr_annotations(data: List[dict], require_ground_truth: bool = False) -> None:
    ...
    if require_ground_truth and with_gt == 0:
        raise RuntimeError(
            "This CIRR JSON contains no target_hard/target_soft ground truth. "
            "Use the CIRR train/validation JSON for local metrics."
        )
...
def evaluate_cross_modal(
    ...
    *score = alpha * sim(text, gallery)*
    ...
    for item in tqdm(data, desc=f"{model_name}: ranking"):
        ref_id = item["reference_id"]
        positives = set(item.get("positives") or [])
        if not positives and item.get("target_hard"):
            positives.add(item["target_hard"])
        if not positives and item.get("target_id"):
            positives.add(item["target_id"])
        ...
        for alpha in alphas:
            ...
            for item in tqdm(
                ...
                update_metrics(
                    results,
                    ranked_ids,
                    (item.get("positives") or
                     ([item["target_hard"]] if item.get("target_hard") else
                      ([item["target_id"]] if item.get("target_id") else []))),
                )
                total += 1
...
def main():
    ...
    if not all_results:
        positives_count = sum(
            1 for x in data if x.get("positives") or x.get("target_id")
        )
