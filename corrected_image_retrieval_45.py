...
def load_qwen3_model(model_path: str, query_instruction: str, max_length: int = 8192,
    ...
    """Load Qwen3-VL-Embedding locally.

    Preferred backend: official Qwen3VLEmbedder wrapper.
    Fallback backend: local SentenceTransformer multimodal wrapper, matching
    the model card usage for text/image/text+image inputs.
    """
    if not model_path:
    ...
    if not (local_path / "config.json").is_file():
        raise FileNotFoundError(f"Missing config.json in Qwen3 model directory: {local_path}")
    ...
    if qwen3_dtype == "float16":
        ...
        raise ValueError("--qwen3_dtype must be auto, float16, bfloat16, or float32")

    if use_flash_attention:
        kwargs["attn_implementation"] = "flash_attention_2"

    # ------------------------------------------------------------------
    # Backend 1: official/local Qwen3VLEmbedder
    # ------------------------------------------------------------------
    try:
        ...
        print(f"Qwen3 backend     : official Qwen3VLEmbedder")
        print(f"Qwen3 checkpoint  : {local_path}")
        print(f"Qwen3 query inst. : {query_instruction}")
        return model
    except Exception as official_exc:
        print(
            "[WARN] Official Qwen3VLEmbedder loader failed; "
            "trying local SentenceTransformer backend."
        )
        print(f"[WARN] Official loader: {type(official_exc).__name__}: {official_exc}")

    # ------------------------------------------------------------------
    # Backend 2: Sentence-Transformers multimodal embedding
    # The supplied model card explicitly documents this local model family
    # with SentenceTransformer.encode() for text/image/text+image inputs.
    # ------------------------------------------------------------------
    try:
        ...
        # Do not force low precision on CPU. Let SentenceTransformers select
        # a compatible dtype unless the user explicitly requested one.
        if qwen3_dtype in {"float16", "bfloat16", "float32"}:
            st_kwargs["torch_dtype"] = kwargs["torch_dtype"]
        ...
        raise RuntimeError(
            "Could not load local Qwen3-VL-Embedding model.\n\n"
            f"Model path: {local_path}\n"
            f"Official backend error: {type(official_exc).__name__}: {official_exc}\n"
            f"SentenceTransformer backend error: {type(st_exc).__name__}: {st_exc}\n\n"
            "For this local checkpoint, make sure sentence-transformers is installed "
            "and the model directory contains the Sentence-Transformers multimodal modules."
        ) from st_exc
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=["clip"],
        choices=["clip", "searle", "open_clip", "siglip", "clip_beta", "blip", "qwen3"],
    )
    ...
    if "qwen3" in args.models:
        ...
        qwen3_model_key = (
            f"qwen3_vl_embedding_{hashlib.sha1((str(Path(args.qwen3_path).resolve()) + "|qwen3").encode('utf-8')).hexdigest()[:12]}"
        )
