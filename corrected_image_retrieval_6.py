...
def load_searle_model(clip_model_name: str = "ViT-B/32"):
    """
    Robust SEARLE loader, especially for Windows.

    The SEARLE hubconf.py imports:
        from src.encode_with_pseudo_tokens import encode_with_pseudo_tokens

    A stale/incomplete torch.hub cache can therefore fail before the model
    itself is loaded. We validate the cached repository and automatically
    rebuild the cache when necessary.
    """
    import sys

    hub_root = Path(torch.hub.get_dir())
    repo_dir = hub_root / "miccunifi_SEARLE_main"
    ...
    def cache_is_valid() -> bool:
        return (
            repo_dir.is_dir()
            and hubconf_file.is_file()
            and src_dir.is_dir()
            and encoder_file.is_file()
        )

    def remove_broken_cache() -> None:
        if repo_dir.exists():
            print(f"Removing broken SEARLE cache: {repo_dir}")
            shutil.rmtree(repo_dir, ignore_errors=True)

    prepare_import_path()

    # First attempt: use a valid cache.
    if cache_is_valid():
        try:
            ...
            return searle.to(DEVICE).eval(), encode_with_pseudo_tokens
        except (ModuleNotFoundError, ImportError) as exc:
            if "src" not in str(exc) and "encode_with_pseudo_tokens" not in str(exc):
                raise
            print(
                "Detected an unusable SEARLE cache. "
                "The repository will be downloaded again."
            )
            remove_broken_cache()

    # Second attempt: force a clean repository download.
    prepare_import_path()
    ...
    try:
        ...
        raise RuntimeError(
            "Unable to load SEARLE after rebuilding its torch.hub cache.\n"
            f"Expected cache directory: {repo_dir}\n"
...
def main():
    ...
    if "searle" in args.models:
        ...
        try:
            ...
            searle, encode_with_pseudo_tokens = load_searle_model(args.clip_model)
