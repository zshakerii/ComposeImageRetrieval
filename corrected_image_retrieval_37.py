...
def evaluate_blip(
    ...
    for start in tqdm(
        ...
        # Ensure features is a tensor, not a tuple
        if isinstance(features, tuple):
            features = features[0]

        # Store each feature individually with proper shape (1, feature_dim)
        for i, caption in enumerate(batch):
            text_cache[caption] = features[i:i + 1]  # Keep 2D shape
    ...
    for ref_id in tqdm(unique_refs, desc="BLIP reference embeddings"):
        ...
        if image is None:
            ...
            # Ensure it's a tensor, not a tuple
            if isinstance(ref_feat, tuple):
                ref_feat = ref_feat[0]
            ref_cache[ref_id] = ref_feat
    ...
    for alpha in alphas:
        ...
        for item in tqdm(data, desc=f"BLIP alpha={alpha:.2f}"):
            ...
            try:
                # Validate that features are tensors
                if not isinstance(text_feat, torch.Tensor):
                    raise TypeError(f"text_feat is {type(text_feat)}, expected torch.Tensor")
                if not isinstance(ref_feat, torch.Tensor):
                    raise TypeError(f"ref_feat is {type(ref_feat)}, expected torch.Tensor")
