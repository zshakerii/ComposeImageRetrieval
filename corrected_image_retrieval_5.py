...
def load_searle_model(clip_model_name: str = "ViT-B/32"):
    """Load official SEARLE and make its cached `src` package importable on Windows."""
    import sys
    ...
    try:
        ...
        if "src" in str(exc):
            raise RuntimeError(
                f"SEARLE cache is incomplete/unimportable: {repo_dir}. "
                "Delete that cache directory and run again."
            ) from exc
        raise
