...
def parse_cirr_sample(sample: dict) -> dict:
    ...
    target_id = normalize_id(sample["target_hard"])
    members = [
        normalize_id(x)
        for x in sample.get("img_set", {}).get("members", [])
    ]
...
def load_dataset(json_path: str) -> Tuple[str, List[dict]]:
    ...
    if isinstance(data, dict) and "annotations" in data:
        return "circo", [parse_circo_sample(x) for x in data["annotations"]]

    if isinstance(data, list) and data:
        first = data[0]

        if "reference_img_id" in first and "target_img_id" in first:
            return "circo", [parse_circo_sample(x) for x in data]

        if "reference" in first and "img_set" in first and "target_hard" in first:
            return "cirr", [parse_cirr_sample(x) for x in data]

    raise ValueError("Unknown CIRR/CIRCO JSON format.")
...
def evaluate_searle(
    ...
    SEARLE itself produces one composed query embedding per sample.
    Ranking is then a cosine/IP search against normalized gallery
    image embeddings.
    """
    ...
    for item in tqdm(data, desc="SEARLE: ranking"):
        ...
        try:
            query_feat = searle_query_embedding(
            ...
            skipped += 1
            print(
                f"[WARN] SEARLE query failed for "
                f"{reference_id}: {exc}"
            )
...
def main():
    ...
    if "searle" in args.models:
        ...
        try:
            ...
            print(f"[ERROR] SEARLE evaluation failed: {exc}")
            traceback.print_exc()
