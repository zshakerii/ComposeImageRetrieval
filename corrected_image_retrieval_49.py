...
def evaluate_blip(
    ...
    """Evaluate BLIP without alpha sweeps.*

    *CIRR query = fixed parameter-free composition of the reference image and**
    *relative caption in BLIP's shared projected embedding space. Optional ITM**
    *reranking is applied to only the first ``itm_topk`` candidates.**
    *"""
    id2idx = build_id_index(gallery_ids)

    if not torch.is_tensor(gallery_feats):
        raise TypeError(
            f"gallery_feats must be torch.Tensor, got {type(gallery_feats).__name__}. "
            "stack_feature_cache() must be unpacked as (ids, features)."
        )
    ...
    for start in tqdm(
        ...
        if not torch.is_tensor(features):
            raise TypeError(
                f"BLIP text encoder returned {type(features).__name__}, expected Tensor."
            )
        if features.ndim != 2 or features.shape[0] != len(batch):
            raise ValueError(
                f"Invalid BLIP text feature shape {tuple(features.shape)} for batch size {len(batch)}."
            )
        for i, caption in enumerate(batch):
            text_cache[caption] = features[i:i + 1].float().cpu()

    # Cache unique reference images once.
    unique_refs = sorted({
    ...
    for ref_id in tqdm(unique_refs, desc="BLIP reference embeddings"):
        ...
        if image is None:
            ...
            continue
        try:
            ref_feat = blip_image_features([image], model, processor)
            if not torch.is_tensor(ref_feat):
                raise TypeError(
                    f"BLIP image encoder returned {type(ref_feat).__name__}, expected Tensor."
                )
            if ref_feat.ndim != 2 or ref_feat.shape[0] != 1:
                raise ValueError(
                    f"Invalid reference BLIP feature shape {tuple(ref_feat.shape)} for {ref_id}."
                )
            ref_cache[ref_id] = ref_feat.float().cpu()
        except Exception as exc:
            print(
                f"[WARN] BLIP reference embedding failed: "
                f"reference={ref_id!r}: {type(exc).__name__}: {exc}"
            )
            ref_cache[ref_id] = None
    ...
    for item in tqdm(data, desc="BLIP retrieval"):
        ...
        try:
            if ref_feat.ndim != 2 or text_feat.ndim != 2:
                raise ValueError(
                    f"BLIP query features must be 2-D; ref={tuple(ref_feat.shape)}, "
                    f"text={tuple(text_feat.shape)}"
                )
            if ref_feat.shape[1] != gallery_feats.shape[1]:
                raise ValueError(
                    f"BLIP reference feature dimension {ref_feat.shape[1]} "
                    f"!= gallery dimension {gallery_feats.shape[1]}"
                )
            if text_feat.shape[1] != gallery_feats.shape[1]:
                raise ValueError(
                    f"BLIP text feature dimension {text_feat.shape[1]} "
                    f"!= gallery dimension {gallery_feats.shape[1]}"
                )

            # No alpha: fixed equal composition of reference image + caption.
            query = F.normalize(
...
def evaluate_qwen3(
    ...
    """Evaluate CIRR/CIRCO with Qwen3 joint image+text query embeddings."""
    id2idx = build_id_index(gallery_ids)
    ...
    if gallery_feats.shape[0] != len(gallery_ids):
        raise ValueError(
            f"Qwen3 gallery mismatch: {gallery_feats.shape[0]} features vs "
            f"{len(gallery_ids)} IDs."
        )
    ...
    # Reference images are often repeated across several CIRR queries. Resolve paths
    # once to avoid repeated filesystem traversal.
    ...
    for item in tqdm(data, desc="Qwen3 composed retrieval"):
        ...
        if not ref_id:
            skipped += 1
            skip_reasons["missing_reference_id"] = skip_reasons.get("missing_reference_id", 0) + 1
            continue
        if not caption:
            skipped += 1
        ...
        if ref_path is None:
            skipped += 1
            skip_reasons["missing_reference_image"] = skip_reasons.get("missing_reference_image", 0) + 1
            continue
        ...
        try:
            ...
            skip_reasons[reason] = skip_reasons.get(reason, 0) + 1
            print(
                f"[WARN] Qwen3 failed for query={query_key!r}, "
                f"reference={ref_id!r}: {reason}"
            )
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=["clip"],
        choices=["clip", "searle", "open_clip", "siglip", "clip_beta", "blip", "align", "qwen3"],
    )
    ...
    parser.add_argument(
        "--qwen3_path",
        default=None,
        help="Local Qwen3-VL-Embedding-2B model directory.",
    )
    parser.add_argument(
        "--qwen3_query_instruction",
        default=QWEN3_DEFAULT_QUERY_INSTRUCTION,
        help="Instruction applied to composed CIRR image+text queries.",
    )
    parser.add_argument(
        "--qwen3_image_batch_size",
        type=int,
        default=1,
        help="Gallery image batch size for Qwen3. Use 1-2 on small GPUs.",
    )
    parser.add_argument(
        "--qwen3_max_length",
        type=int,
        default=8192,
        help="Maximum Qwen3 multimodal sequence length.",
    )
    parser.add_argument(
        "--qwen3_dtype",
    ...
    parser.add_argument(
        "--qwen3_flash_attention",
        action="store_true",
        help="Use FlashAttention-2 when supported by the local environment.",
    )
    ...
    parser.add_argument(
        "--align_path",
        default="./models_download/align-base",
        help=(
            "Local ALIGN checkpoint directory. Usually a local copy of "
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Batch size for ALIGN full-gallery image embeddings.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Force rebuilding persistent gallery image embeddings instead of using the cache.",
    )
    parser.add_argument(
        "--open_clip_model",
        default="ViT-H-14",
    ...
    if args.open_clip_batch_size < 1 or args.siglip_batch_size < 1 or args.align_batch_size < 1:
        raise ValueError("Batch sizes must be >= 1.")
    if "align" in args.models and not args.align_path:
        raise ValueError("--align_path is required when --models includes align.")
    if args.qwen3_image_batch_size < 1:
        raise ValueError("--qwen3_image_batch_size must be >= 1.")
    if args.qwen3_max_length < 128:
        raise ValueError("--qwen3_max_length must be >= 128.")
    if "qwen3" in args.models and not args.qwen3_path:
        raise ValueError("--qwen3_path is required when --models includes qwen3.")
    ...
    if "blip" in args.models:
        ...
        print("BLIP query composition: normalize(reference + caption); alpha sweep disabled.")
        print(
            "BLIP ITM reranking: "
        ...
        # CRITICAL BUG FIX:
        # stack_feature_cache() returns (ids, features). The old code assigned
        # the entire tuple to blip_gallery_feats, causing `tuple @ Tensor`.
        blip_gallery_ids, blip_gallery_feats = stack_feature_cache(blip_cache)

        results, total, skipped, rankings, skip_reasons = evaluate_blip(
            data=data,
            image_folder=args.image_folder,
            reference_image_folder=reference_image_folder,
        ...
        if any(bool(get_relevant_ids(x)) for x in data):
            ...
            print_metric_summary(tag, summary, total, skipped)
        ...
        if split_name == "test":
            ...
            save_cirr_test_predictions(
                ...
                extra={
                    ...
                    "composition": "normalize(reference_embedding + text_embedding)",
                },
            )
    ...
    if "align" in args.models:
        print("\n=== ALIGN ===")
        print("Checkpoint: kakaobrain/align-base")
        print("ALIGN query composition: alpha * caption + (1-alpha) * reference image")

        align_model, align_processor = load_align_model(args.align_path)

        resolved_align_path = str(Path(args.align_path).expanduser().resolve())
        try:
            config_stat = (Path(resolved_align_path) / "config.json").stat()
            align_fingerprint_source = (
                f"{resolved_align_path}|{config_stat.st_size}|{config_stat.st_mtime_ns}"
            )
        except OSError:
            align_fingerprint_source = resolved_align_path

        align_hash = hashlib.sha1(
            align_fingerprint_source.encode("utf-8")
        ).hexdigest()[:12]
        align_model_key = f"align_base_{align_hash}"
        ...
        align_cache = build_align_image_cache(
            ...
            dataset=args.dataset,
            split=split_name,
            model_key=align_model_key,
            batch_size=args.align_batch_size,
            force_rebuild=args.force_rebuild_embeddings,
        )

        align_gallery_ids, align_gallery_feats = stack_feature_cache(align_cache)
        print(f"ALIGN gallery embeddings: {len(align_gallery_ids)}")
        ...
        for alpha in args.alphas:
            results, total, skipped, rankings, skip_reasons = evaluate_cross_modal(
                ...
                image_folder=args.image_folder,
                reference_image_folder=reference_image_folder,
                gallery_ids=align_gallery_ids,
                gallery_feats=align_gallery_feats,
                get_image_feature=lambda img: align_image_embedding(
                    img, align_model, align_processor
                ),
                get_text_feature=lambda txt: align_text_embedding(
                    txt, align_model, align_processor
                ),
                model_name="ALIGN",
                alpha=alpha,
                subset_protocol=subset_protocol,
            ...
            if split_name == "test":
                output_path = Path(args.json_path).with_name(
                    Path(args.json_path).stem
                    + f"_align_a{alpha:.2f}_predictions.json"
                )
                save_cirr_test_predictions(
                    path=output_path,
                    data=data,
                    ...
                    extra={
                        "alpha": alpha,
                        "model_checkpoint": resolved_align_path,
                    },
                )

        del align_model, align_processor, align_cache, align_gallery_feats
        if DEVICE.type == "cuda":
            torch.cuda.empty_cache()
    ...
    if "qwen3" in args.models:
        ...
        qwen3_model = _make_qwen3_process_adapter(
            qwen3_model,
            args.qwen3_query_instruction,
        )

        qwen3_cache_seed = str(Path(args.qwen3_path).resolve()) + "|qwen3"
        qwen3_cache_hash = hashlib.sha1(
            qwen3_cache_seed.encode("utf-8")
        ).hexdigest()[:12]
        qwen3_model_key = f"qwen3_vl_embedding_{qwen3_cache_hash}"
        qwen3_cache = build_qwen3_image_cache(
            gallery_ids,
            args.image_folder,
            qwen3_model,
            cache_dir=args.embedding_cache_dir,
            dataset=args.dataset,
            split=split_name,
            model_key=qwen3_model_key,
            batch_size=args.qwen3_image_batch_size,
            force_rebuild=args.force_rebuild_embeddings,
        )
        qwen3_gallery_ids, qwen3_gallery_feats = stack_feature_cache(qwen3_cache)

        results, total, skipped, rankings, skip_reasons = evaluate_qwen3(
            data=data,
            gallery_ids=qwen3_gallery_ids,
            gallery_feats=qwen3_gallery_feats,
            reference_image_folder=reference_image_folder,
        ...
        if has_gt:
            summary = summarize_results(results)
            all_results["Qwen3-VL-Embedding-2B"] = summary
            print_metric_summary(
                "Qwen3-VL-Embedding-2B", summary, total, skipped
            )

        # Save predictions for every split so Qwen3 can be analyzed independently.
        ...
        if args.dataset == "cirr":
                ...
                "predictions": [
                    {
                        ...
                        "target_id": item.get("target_id"),
                        "caption": item.get("caption", ""),
                        "ranking": rankings.get(get_query_key(item), []),
                    }
                    for item in data
                ],
            }
            with open(qwen_output, "w", encoding="utf-8") as f:
                json.dump(prediction_payload, f, ensure_ascii=False, indent=2)
            print(f"Predictions saved to: {qwen_output}")

        del qwen3_model, qwen3_cache, qwen3_gallery_feats
        if DEVICE.type == "cuda":
            torch.cuda.empty_cache()
