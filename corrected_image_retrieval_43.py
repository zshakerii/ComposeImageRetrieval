...
def _load_qwen3_embedder_class(model_path: Optional[str] = None):
    """Load the official/local Qwen3-VL-Embedding embedder implementation."""
    candidates = []
    if model_path:
        root = Path(model_path).expanduser().resolve()
        candidates.extend([
            root / "scripts" / "qwen3_vl_embedding.py",
            root / "src" / "models" / "qwen3_vl_embedding.py",
            root / "qwen3_vl_embedding.py",
        ])
    candidates.extend([
    ...
    for file_path in candidates:
        ...
        try:
            spec = importlib.util.spec_from_file_location(
                "qwen3_vl_embedding_local", str(file_path.resolve())
            )
    ...
    # Final fallback: installed package/module.
    import importlib
    for module_name in (
    ...
    raise ImportError(
        "Could not import Qwen3VLEmbedder. Expected the official/local "
        "qwen3_vl_embedding.py under ./scripts/ or ./src/models/.\n" +
        "\n".join(errors)
    )
...
def load_qwen3_model(model_path: str, query_instruction: str, max_length: int = 8192,
    ...
    """Load Qwen3-VL-Embedding-2B completely from a local directory."""
    if not model_path:
    ...
    Embedder = _load_qwen3_embedder_class(str(local_path))
    ...
    # The official implementation accepts instruction per input, therefore the
    # loader itself keeps the model-neutral instruction out of the constructor.
    model = Embedder(
    ...
    print(f"Qwen3 device     : {getattr(model.model, 'device', DEVICE)}")
    print(f"Qwen3 dtype      : {getattr(model.model, 'dtype', 'unknown')}")
    print(f"Qwen3 query inst.: {query_instruction}")
    return model


def _qwen3_model_process(model, items, normalize=True):
    """Call the official embedder and normalize to CPU Tensor."""
    if not items:
        return torch.empty((0, 0), dtype=torch.float32)
...
def main():
    ...
    if "qwen3" in args.models:
        ...
        qwen3_model_key = (
            f"qwen3_vl_embedding_2b_{hashlib.sha1(str(Path(args.qwen3_path).resolve()).encode('utf-8')).hexdigest()[:12]}"
        )
