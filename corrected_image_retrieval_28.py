...
def _siglip_forward_image_features(inputs, model) -> torch.Tensor:
    ...
    Newer Transformers versions may make model.get_image_features() return
    BaseModelOutputWithPooling. Calling the full model exposes image_embeds,
    which are the projected embeddings used for image-text similarity.
    """
    try:
    ...
    # Some current SigLIP2 versions expose the vision encoder output here.
    # In that case get the pooled output and apply the model projection.
    pooler = getattr(output, "pooler_output", None)
    if torch.is_tensor(pooler):
        vision_model = getattr(model, "vision_model", None)
        if vision_model is not None:
            projection = getattr(vision_model, "head", None)
        ...
        if image_projection is not None:
            return image_projection(pooler)

    raise TypeError(
        "Unsupported SigLIP image output: "
        f"{type(output).__name__}. Expected Tensor, image_embeds, or pooler_output."
    )
...
def _siglip_forward_text_features(inputs, model) -> torch.Tensor:
    ...
    if torch.is_tensor(pooler):
        ...
        if text_model is not None:
            projection = getattr(text_model, "head", None)
            if projection is not None:
                return projection(pooler)
            projection = getattr(text_model, "visual_projection", None)
            if projection is not None:
                return projection(pooler)
    ...
    raise TypeError(
        ...
        f"{type(output).__name__}. Expected Tensor, text_embeds, or pooler_output."
    )
