...
try:
    ...
    raise ImportError(
        "The OpenAI CLIP package is required. Install the CLIP package/repository "
...
try:
    ...
    raise ImportError(
        "open_clip_torch is required for the --models open_clip option."
    ) from exc
...
def load_siglip_model(model_path: str):
    ...
    # Model is already local: do not query/download from Hugging Face Hub.
    os.environ.setdefault("HF_HUB_OFFLINE", "1")
    os.environ.setdefault("TRANSFORMERS_OFFLINE", "1")

    local_path = Path(model_path).expanduser().resolve()
    if not local_path.is_dir():
        raise FileNotFoundError(f"SigLIP local model directory not found: {local_path}")
    if not (local_path / "config.json").is_file():
        raise FileNotFoundError(f"Missing config.json in local SigLIP directory: {local_path}")
...
def evaluate_cross_modal(
    ...
    for item in tqdm(data, desc=f"{model_name}: ranking"):
        ...
        try:
            ...
            if alpha is None:
                # Alpha-free CIRR composition for SigLIP:
                # combine reference-image and relative-caption embeddings
                # directly in the shared SigLIP space.
                #
                # q = normalize(reference + caption)
                # score(I) = cosine(I, q)
                # This keeps both modalities active without introducing a
                # tunable alpha hyperparameter.
                query_feat = F.normalize(ref_feat + text_feat, dim=-1)
                sims = gallery_feats @ query_feat.squeeze(0)
            else:
...
def main():
    ...
    parser.add_argument(
        "--siglip_checkpoint_every",
        type=int,
        default=8,
        help="Save SigLIP partial cache every N batches.",
    )
    parser.add_argument(
        "--torch_threads",
        type=int,
        default=0,
        help="CPU torch threads; 0 keeps PyTorch default.",
    )
    ...
    if args.torch_threads > 0 and DEVICE.type == "cpu":
        ...
        try:
            ...
            pass
        print(f"Torch CPU threads: {args.torch_threads}")
    ...
    if "siglip" in args.models:
        ...
        sig_cache = build_siglip_image_cache(
            ...
            force_rebuild=args.force_rebuild_embeddings,
            checkpoint_every_batches=args.siglip_checkpoint_every,
        )

        sig_ids, sig_feats = stack_feature_cache(sig_cache)

        # SigLIP does NOT sweep alpha.
        # CIRR query = normalized(reference image embedding + relative caption embedding).
        results, total, skipped, rankings, skip_reasons = evaluate_cross_modal(
            data=data,
            image_folder=args.image_folder,
            reference_image_folder=reference_image_folder,
        ...
        if has_gt:
            summary = summarize_results(results)
            all_results["SigLIP (alpha-free)"] = summary

            print_metric_summary(
                "SigLIP (alpha-free)", summary, total, skipped
            )

        if split_name == "test":
            output_path = Path(args.json_path).with_name(
                Path(args.json_path).stem + "_siglip_alpha_free_predictions.json"
            )
            save_cirr_test_predictions(
                path=output_path,
                data=data,
                model_name="SigLIP (alpha-free)",
                split=split_name,
                rankings=rankings,
                total=total,
                skipped=skipped,
                skip_reasons=skip_reasons,
                extra={"alpha": None, "composition": "normalize(reference + caption)"},
            )
